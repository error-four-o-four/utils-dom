...
const createElement = (...args: string[]): never | HTMLElement => {
  ...
  const tagName: string = args.reduce(
    (prev: string, curr: string) =>
      !curr.match(/#/) && !curr.match(/\./) ? curr : prev,
    args[0]
  );
 ...
 */
export const remove = (
  ...args: string[] | HTMLElement[] | [HTMLElement | HTMLElement[]]
): void => {
  ...
  for (const arg of args)
    Array.isArray(arg) ? values.push(...arg) : values.push(arg);
